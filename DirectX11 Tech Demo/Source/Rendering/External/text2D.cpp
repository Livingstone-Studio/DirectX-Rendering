...
Text2D::Text2D(string filename, ID3D11Device* device, ID3D11DeviceContext* context)
{
	...
	hr = D3DCompileFromFile(L"Source/Rendering/Shaders/text2d_shaders.hlsl", 0, 0, "TextVS", "vs_4_0", 0, 0, &VS, &error);
	if (FAILED(hr))
	{
	...
	hr = D3DCompileFromFile(L"Source/Rendering/Shaders/text2d_shaders.hlsl", 0, 0, "TextPS", "ps_4_0", 0, 0, &PS, &error);
	if (FAILED(hr))
	{
		OutputDebugStringA(reinterpret_cast<const char*>(error->GetBufferPointer()));
		error->Release();
